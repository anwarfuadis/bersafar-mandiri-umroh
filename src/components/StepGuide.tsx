  ...
  return <section className="relative py-20 overflow-hidden bg-gradient-to-b from-sand-50 to-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          {/* Navigation Buttons */}
          <button onClick={() => scrollToStep('left')} className={`absolute left-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white shadow-lg border border-sand-200 transition-all duration-300 ${canScrollLeft ? 'hover:bg-sand-50 text-spiritual-600' : 'opacity-50 cursor-not-allowed text-spiritual-300'}`} disabled={!canScrollLeft}>
            <ChevronLeft size={24} />
          </button>

          <button onClick={() => scrollToStep('right')} className={`absolute right-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white shadow-lg border border-sand-200 transition-all duration-300 ${canScrollRight ? 'hover:bg-sand-50 text-spiritual-600' : 'opacity-50 cursor-not-allowed text-spiritual-300'}`} disabled={!canScrollRight}>
            <ChevronRight size={24} />
          </button>

          {/* Progress Line */}
          <div className="absolute top-16 left-12 right-12 h-1 bg-sand-200 rounded-full">
            <div className="h-full bg-gradient-to-r from-spiritual-600 to-gold-500 rounded-full transition-all duration-500" style={{
          ...
          msOverflowStyle: 'none'
            ...
            {steps.map((step, index) => <div key={step.id} className={`flex-shrink-0 w-64 cursor-pointer transition-all duration-300 ${activeStep === step.id ? 'scale-105' : 'hover:scale-102'}`} onClick={() => setActiveStep(step.id)} style={{
            animationDelay: `${index * 0.1}s`
          }}>
                {/* Step Circle - Fixed cropping issue */}
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 mx-auto transition-all duration-300 ${activeStep === step.id ? 'bg-gradient-to-r from-spiritual-600 to-spiritual-700 text-white shadow-lg transform scale-110' : activeStep > step.id ? 'bg-gold-400 text-white' : 'bg-sand-200 text-spiritual-600'} overflow-visible`}>
                  <span className="text-2xl">{step.icon}</span>
                </div>

                {/* Step Content */}
                <div className={`text-center transition-all duration-300 ${activeStep === step.id ? 'text-spiritual-800' : 'text-spiritual-600'}`}>
        ...
        <div className="text-center mt-16 animate-fade-in">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-sand-200">
            <h3 className="text-2xl font-sf font-semibold text-spiritual-800 mb-4">Siap Umrah Mandiri Sekarang?</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-gradient-to-r from-spiritual-600 to-spiritual-700 hover:from-spiritual-700 hover:to-spiritual-800 text-white px-8 py-3 text-lg font-sf font-medium rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
                Daftar Sekarang
              </Button>
              <Button variant="outline" size="lg" className="border-2 border-spiritual-300 text-spiritual-700 hover:bg-spiritual-50 px-8 py-3 text-lg font-sf font-medium rounded-2xl transition-all duration-300">
                Konsultasi Gratis
              </Button>
            </div>
          </div>
