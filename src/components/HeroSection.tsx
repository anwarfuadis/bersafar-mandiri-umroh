  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-30 z-10" style={{
        ...
        <div className="absolute top-20 left-10 w-32 h-32 border-4 border-gold-300/70 rounded-full animate-pulse-gentle shadow-xl" style={{
          transform: `translateY(${scrollY * 0.3}px)`
        }}></div>
        <div className="absolute bottom-40 right-20 w-24 h-24 border-4 border-sand-200/70 rounded-full animate-pulse-gentle shadow-xl" style={{
          animationDelay: '1s',
          transform: `translateY(${scrollY * 0.4}px)`
        }}></div>
        <div className="absolute top-1/2 left-1/4 w-16 h-16 border-4 border-gold-200/70 transform rotate-45 animate-pulse-gentle shadow-xl" style={{
          animationDelay: '2s',
          transform: `rotate(45deg) translateY(${scrollY * 0.2}px)`
        }}></div>
        
        {/* Additional geometric shapes */}
        <div className="absolute top-1/3 right-1/6 w-20 h-20 border-4 border-gold-400/60 transform rotate-12 shadow-xl" style={{
          transform: `rotate(12deg) translateY(${scrollY * 0.35}px)`
        }}></div>
        <div className="absolute bottom-1/3 left-1/6 w-28 h-28 border-4 border-sand-300/60 rounded-full shadow-xl" style={{
          transform: `translateY(${scrollY * 0.25}px)`
        }}></div>
        <div className="absolute top-1/4 left-2/3 w-12 h-12 bg-gold-300/40 transform rotate-45 shadow-xl" style={{
          transform: `rotate(45deg) translateY(${scrollY * 0.45}px)`
        }}></div>
        <div className="absolute bottom-1/4 right-2/3 w-36 h-36 border-4 border-spiritual-400/50 rounded-full shadow-xl" style={{
